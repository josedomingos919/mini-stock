...
class Application extends Aux
{
    ...
    public function login($_DATA)
    {
        if (!(isset($_DATA['username']) && isset($_DATA['password']))) {
            echo json_encode([
                'info' => 'Esperava receber nome de usuário e senha!',
                'data' => null,
                'status' => false,
            ]);
            return;
    ...
    public function deleteLink($data)
    {
        ...
        $data = $db->query(
            "select fileName, fullname from vwLinkpassword where $query_text ;"
        );
    ...
    public function resetPassword($_DATA)
    {
        ...
        if (!isset($_DATA['email'])) {
            echo json_encode([
                'info' => 'Esperava receber um email',
                'data' => null,
                'status' => false,
            ]);
            exit();
        ...
        $fullName =
            $_SERVER['DOCUMENT_ROOT'] . "/Unibega/api/ResetPassword/$name";

        $db->query(
            " insert into linkPassword values (default, '$fileName ','$fullName', '$email', default); "
        );
        ...
        $res = $this->sendEmail([
            ...
            'toName' => $_DATA['email'],
            'subject' =>
                'UNIBEGA, link para fazer a redefinição da sua senha!...',
            'body' => "
                    link:  
